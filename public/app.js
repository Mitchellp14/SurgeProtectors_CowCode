...
function renderGraphs(labels, temp, hum, meth, co2, feedWeight, cattleWeight, dates) {
    ...
    if (mode === "single") {
        ...
        if (singleChart && singleChart.config.data.datasets[0].label === labelToShow) {
            // Updates the X-axis timestamps
            singleChart.data.labels = labels;
            // Updates the Y-axis data points for the selected metric
            singleChart.data.datasets[0].data = dataToShow;
            // Redraws the chart smoothly without re-triggering the load animation
            singleChart.update('none'); 
        } else {
            // If it exists but is a different metric, destroy it completely
            if (singleChart) singleChart.destroy();
            
            // Generate a brand new chart and assign it to our dedicated variable
            singleChart = new Chart(ctx, {
                // Instructs Chart.js to draw a line graph
                type: "line",
                data: {
                    // Sets the horizontal axis labels
                    labels: labels,
                    datasets: [{
                        // Sets the label for the hover tooltip
                        label: labelToShow,
                        // Loads the actual metric data points
                        data: dataToShow,
                        // Sets the line color
                        borderColor: colorToShow,
                        // Creates a semi-transparent fill under the line
                        backgroundColor: colorToShow.replace('1)', '0.1)'),
                        // Sets the thickness of the line
                        borderWidth: 2,
                        // Tells the chart to fill the area under the line
                        fill: true,
                        // Adds a slight curve to the line instead of rigid angles
                        tension: 0.3
                    }]
                },
                options: {
                    // Allows the chart to stretch and fill its HTML container
                    maintainAspectRatio: false,
                    plugins: {
                        tooltip: {
                            callbacks: {
                                // Customizes the bold title of the hover tooltip
                                title: function (tooltipItems) {
                                    // Finds which specific data point the user is hovering over
                                    const index = tooltipItems[0].dataIndex;
                                    // Combines the exact date and time for absolute clarity
